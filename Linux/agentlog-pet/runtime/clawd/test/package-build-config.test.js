...
describe("package build config", () => {
  ...
  it("uses the product brand as its only application icon source", () => {
    assert.strictEqual(pkg.build.linux.icon, "../../assets/brand/icons");
    assert.strictEqual(pkg.build.mac.icon, "../../assets/brand/icon.png");
    assert.strictEqual(pkg.build.win.icon, "../../assets/brand/icon.png");
    assert.ok(!pkg.build.files.includes("assets/icons/**/*"));
    assert.ok(!pkg.build.files.some((entry) => /dock-icon|tray-icon\*|assets\/icon\./.test(entry)));
