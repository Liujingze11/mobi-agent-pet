...
function readTrackedTree(repoRoot) {
  ...
  const raw = execFileSync("git", ["ls-files", "--full-name", "--stage", "-z"], {
    cwd: repoRoot,
    encoding: "utf8",
    maxBuffer: 32 * MIB,
  });
  return raw.split("\0")
    ...
    .map((entry) => {
      ...
      if (!match || (prefix && !match[2].startsWith(prefix))) return null;
      const filePath = prefix ? match[2].slice(prefix.length) : match[2];
