...
async function main() {
  ...
  try {
    ...
    for (const [key, expected] of Object.entries({ Id: "com.agentlog.pet.tray", Title: "Mobi Agent Pet",
    ...
    assert.equal(props.IconThemePath, path.resolve(path.dirname(helper), "../icons"));
    assert.equal(
      props.IconName,
      path.join(props.IconThemePath, "hicolor", "22x22", "status", "agentlog-pet.png"),
    );
    const theme = fs.readFileSync(path.join(props.IconThemePath, "hicolor", "index.theme"), "utf8");
    ...
    for (const size of [16, 22, 32, 48]) {
      for (const name of [
        ...
        "agentlog-pet-attention-panel",
        "agentlog-pet-working",
        "agentlog-pet-working-panel",
        "agentlog-pet-question",
        "agentlog-pet-question-panel",
        "agentlog-pet-error",
        "agentlog-pet-error-panel",
      ]) {
        const png = fs.readFileSync(path.join(props.IconThemePath, "hicolor", `${size}x${size}`, "status", `${name}.png`));
        assert.equal(png.subarray(0, 8).toString("hex"), "89504e470d0a1a0a");
        assert.equal(png.readUInt32BE(16), size);
        assert.equal(png.readUInt32BE(20), size);
      }
    }
    pass("one native item, packaged icon theme, forty sized PNGs, app-owned helper");
