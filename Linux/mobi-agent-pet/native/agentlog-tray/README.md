...
This directory contains the GTK 3/Ayatana AppIndicator helper used by Mobi Agent Pet
...
```bash
node native/agentlog-tray/test/packaged-session.cjs /path/to/packaged/mobi-agent-pet
APPIMAGE_EXTRACT_AND_RUN=1 node native/agentlog-tray/test/packaged-session.cjs Linux/Mobi-Agent-Pet-0.1.0-x64.AppImage
```
...
the `icons` directory beside the helper's staged `bin` directory. The
`hicolor` theme and its status icon directories live below that search root.
