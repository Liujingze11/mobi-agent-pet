...
static gboolean validate_icon(JsonObject *object, GError **error) {
  ...
  if (!g_str_equal(icon, "agentlog-pet") &&
      ...
      !g_str_equal(icon, "agentlog-pet-working") &&
      !g_str_equal(icon, "agentlog-pet-question") &&
      !g_str_equal(icon, "agentlog-pet-error")) {
    return protocol_fail(error, PROTOCOL_INVALID_ICON, "unsupported icon name");
  }
  return TRUE;
}
...
static gchar *icon_path_for_name(const AgentLogTrayState *state,
                                 const gchar *icon);
static void set_indicator_icon(AgentLogTrayState *state, const gchar *icon);
...
static gboolean apply_parent_message(AgentLogTrayState *state, JsonNode *root) {
  ...
  if (g_str_equal(type, "set-icon")) {
    ...
    state->icon_revision = revision;
    set_indicator_icon(
        state, json_object_get_string_member(message, "icon"));
  }
  return TRUE;
}
...
static gchar *derive_icon_theme_root(const gchar *executable) {
  ...
  gchar *candidate = g_build_filename(binary_directory, "..", "icons", NULL);
...
static gchar *icon_path_for_name(const AgentLogTrayState *state,
                                 const gchar *icon) {
  const gchar *filename = "agentlog-pet.png";
  if (g_str_equal(icon, "agentlog-pet-attention")) {
    filename = "agentlog-pet-attention.png";
  } else if (g_str_equal(icon, "agentlog-pet-working")) {
    filename = "agentlog-pet-working.png";
  } else if (g_str_equal(icon, "agentlog-pet-question")) {
    filename = "agentlog-pet-question.png";
  } else if (g_str_equal(icon, "agentlog-pet-error")) {
    filename = "agentlog-pet-error.png";
  }
  return g_build_filename(state->icon_theme_root, "hicolor", "22x22", "status",
                          filename, NULL);
}

static void set_indicator_icon(AgentLogTrayState *state, const gchar *icon) {
  gchar *path = icon_path_for_name(state, icon);
  app_indicator_set_icon(state->indicator, path);
  g_free(path);
}
