...
test("accepts the three status-light icon names", () => {
  for (const icon of ["agentlog-pet-working", "agentlog-pet-question", "agentlog-pet-error"]) {
    assert.equal(validateParentMessage({
      version: 1,
      type: "set-icon",
      revision: 0,
      icon,
    }, { currentRevision: 0 }).icon, icon);
  }
});
