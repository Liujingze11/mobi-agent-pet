...
describe("hit-renderer input layer", () => {
  it("plain single click opens the context menu, not the session HUD", () => {
    const h = createHarness();
    h.pointerup({});
    const names = h.apiCalls.map((c) => c[0]);
    assert.ok(!names.includes("revealSessionHud"), "must not reveal the session HUD");
    assert.ok(!names.includes("focusTerminal"), "must not call focusTerminal");
  });

  it("plain single click opens the context menu after the click window", () => {
    const h = createHarness();
    h.pointerup({});
    h.fireTimer((t) => t.ms === 400);
    assert.ok(h.apiCalls.some((call) => call[0] === "showContextMenu"),
      "single click should open the context menu");
  });

  it("double click keeps the pet reaction instead of opening the context menu", () => {
    const h = createHarness();
    h.pointerup({});
    h.pointerup({});
    h.fireTimer((t) => t.ms === 400);
    assert.ok(h.apiCalls.some((call) => call[0] === "playClickReaction"),
      "double click should keep the pet reaction");
    assert.ok(!h.apiCalls.some((call) => call[0] === "showContextMenu"),
      "double click should not open the single-click menu");
  });
  ...
  it("opens the menu in working state without revealing the session HUD", () => {
    const h = createHarness();
