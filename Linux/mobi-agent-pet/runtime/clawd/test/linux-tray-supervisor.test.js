...
const ICON_THEME_ROOT = "/opt/agentlog/tray/icons";
...
class FakeChild extends EventEmitter {
  ...
  send(message) {
    if (message.type === "host-status") this.registered = message.registered;
...
function createHarness(options = {}) {
  ...
  function nativeVisible() {
    return children.some((child) => !child.exited && child.registered);
  }

  function assertOneBackend() {
    assert.ok(!(nativeVisible() && fallbackActive), "only one tray backend may be visible");
  }

  const fallback = {
    async start(snapshot) {
      assert.strictEqual(nativeVisible(), false, "native tray must be unavailable before fallback starts");
    ...
    setIcon(icon) {
      fallbackEvents.push({ type: "icon", icon });
    },
    async stop() {
...
test("starts native only after separate ready and registered messages and owns revisions", async () => {
  ...
  await harness.supervisor.setIcon("agentlog-pet-error");
  harness.assertOneBackend();
  ...
  assert.deepStrictEqual(harness.children[0].writes.slice(1), [
    ...
    { version: 1, type: "set-icon", revision: 2, icon: "agentlog-pet-error" },
  ]);
  await harness.supervisor.stop();
});
...
test("restores the current status-light icon when the Electron fallback starts", async () => {
  const harness = createHarness();
  await harness.supervisor.start(makeSnapshot("Settings"));
  await harness.supervisor.setIcon("agentlog-pet-question");

  await harness.clock.tick(2000);

  assert.deepStrictEqual(
    harness.fallbackEvents.filter(({ type }) => type === "icon"),
    [{ type: "icon", icon: "agentlog-pet-question" }],
  );
  await harness.supervisor.stop();
});

test("keeps retrying unexpected exits with a bounded delay", async () => {
  const harness = createHarness();
  await harness.supervisor.start(makeSnapshot("Settings"));
  await becomeNative(harness);
  ...
  assert.deepStrictEqual(harness.clock.pendingDelays(), [30_000]);
  assert.deepStrictEqual(harness.supervisor.getHealth(), {
    status: "electron-fallback",
    code: "native-helper-exited",
  });
  await harness.clock.tick(30_000);
  assert.strictEqual(harness.children.length, 5);
  await becomeNative(harness);
  harness.children.at(-1).exit(1, null);
  await settle();
  assert.deepStrictEqual(harness.clock.pendingDelays(), [30_000]);
...
test("renews the native restart budget after a stable running period", async () => {
  const harness = createHarness();
  await harness.supervisor.start(makeSnapshot("Settings"));
  await becomeNative(harness);
  ...
  await harness.clock.tick(60_000);
  harness.children.at(-1).exit(1, null);
  await settle();
  assert.deepStrictEqual(harness.clock.pendingDelays(), [250]);
  await harness.supervisor.stop();
});
...
test("keeps retrying when native and Electron fallback are both unavailable", async () => {
...
test("uses Electron for an X11 no-host result and reports no-host on Wayland", async () => {
  ...
  assert.deepStrictEqual(x11.supervisor.getHealth(), { status: "native", code: null });
  assert.strictEqual(x11.fallback.isActive(), false);
...
test("restores the native X11 tray when the host returns after fallback failed", async () => {
  const harness = createHarness({ sessionType: "x11", fallbackStartFailures: 1 });
  await harness.supervisor.start(makeSnapshot("Settings"));
  const child = harness.children[0];
  await becomeNative(harness, child);

  await emit(harness, child, {
    version: 1, type: "host-status", watcher: false, registered: false,
  });
  await harness.clock.tick(250);
  assert.deepStrictEqual(harness.supervisor.getHealth(), {
    status: "failed", code: "tray-backends-unavailable",
  });
  assert.strictEqual(child.exited, false, "the helper must keep watching for host recovery");
...
test("restores the native tray when a Wayland host returns", async () => {
  ...
  await harness.clock.tick(250);
  assert.deepStrictEqual(harness.supervisor.getHealth(), {
    status: "no-host", code: "status-notifier-host-missing",
  });
  assert.strictEqual(child.exited, false, "the helper must keep watching for host recovery");
