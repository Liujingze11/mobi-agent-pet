...
test("owns one Electron tray and renders, dispatches, and tears down its descriptor menu", () => {
  ...
  const workingIcon = { size: { width: 32, height: 32 } };
  const questionIcon = { size: { width: 32, height: 32 } };
  const errorIcon = { size: { width: 32, height: 32 } };
  const commands = [];
  let menuOpened = 0;
  const backend = createElectronTrayBackend({
    ...
    statusIcons: {
      ...
      "agentlog-pet-working": workingIcon,
      "agentlog-pet-question": questionIcon,
      "agentlog-pet-error": errorIcon,
    },
  ...
  const snapshot = {
    items: [
      ...
      { kind: "checkbox", id: "tray.visibility", label: "Show in Menu Bar", checked: true },
    ],
  };
  ...
  backend.setIcon("agentlog-pet-error");
  backend.setIcon("agentlog-pet");
  assert.deepStrictEqual(tray.images.slice(-2), [errorIcon, normalIcon]);
