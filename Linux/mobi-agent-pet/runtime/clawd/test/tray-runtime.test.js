...
function createHarness(platform, options = {}) {
  ...
  const runtime = createTrayRuntime({
    platform,
    app: options.app || { isPackaged: false, getAppPath: () => "/repo/app" },
    path,
...
test("resolves Linux tray resources from app resources only when packaged", () => {
  ...
  assert.deepEqual(development, {
    ...
    iconThemeRoot: "/repo/app/build/tray/icons",
  });
  assert.deepEqual(packaged, {
    helperPath: "/bundle/resources/tray/bin/agentlog-tray",
    iconThemeRoot: "/bundle/resources/tray/icons",
  });
});
...
test("retains the latest status-light icon set before startup", async () => {
  const { runtime, created } = createHarness("linux");

  await runtime.setAttention(true);
  await runtime.setAttention(false);
  await runtime.setIcon("agentlog-pet-error");
...
test("forwards a status-light icon to the active backend", async () => {
  const { runtime, created } = createHarness("linux");
  const initial = snapshot("Initial");

  await runtime.start(initial);
  await runtime.setIcon("agentlog-pet-question");
...
test("loads Linux fallback status lights from packaged tray resources", async () => {
  const { runtime, created } = createHarness("linux", {
    app: { isPackaged: true, getAppPath: () => "/bundle/app.asar" },
  });

  await runtime.start(snapshot("Initial"));

  const fallbackOptions = created.find((entry) => entry.kind === "electron").options;
  assert.equal(
    fallbackOptions.attentionIcon.source,
    "/resources/tray/icons/hicolor/32x32/status/agentlog-pet-question.png",
  );
  assert.equal(
    fallbackOptions.statusIcons["agentlog-pet-question"].source,
    "/resources/tray/icons/hicolor/32x32/status/agentlog-pet-question.png",
  );
});
