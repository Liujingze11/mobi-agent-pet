...
describe("resolveDisplayState()", () => {
  ...
  it("keeps an active Kimi permission lock visible", () => {
    api.cleanup();
    const ctx = makeCtx({
...
describe("setState() debounce", () => {
  ...
  it("notifies the host when the displayed state changes", () => {
    const changes = [];
    api.cleanup();
    ctx = makeCtx({ onStateChanged: (state) => changes.push(state) });
    api = require("../src/state")(ctx);

    api.setState("working");
    assert.deepStrictEqual(changes, ["working"]);
  });
...
describe("AgentLog event observer", () => {
  ...
  it("notifies the tray after each accepted work event, even when display state is unchanged", () => {
    const observed = [];
    api = require("../src/state")(makeCtx({
      onTrayAgentEvent: (input) => observed.push(input),
    }));

    api.updateSession("claude-1", "working", "PreToolUse", { agentId: "claude-code" });
    api.updateSession("claude-1", "working", "PostToolUse", { agentId: "claude-code" });

    assert.deepStrictEqual(observed.map(({ event }) => event), ["PreToolUse", "PostToolUse"]);
  });
