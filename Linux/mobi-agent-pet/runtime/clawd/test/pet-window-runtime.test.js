...
describe("pet-window-runtime", () => {
  ...
  it("does not expose the retired manual primary-display action", () => {
    const harness = createRuntime();

    assert.strictEqual(harness.runtime.bringPetToPrimaryDisplay, undefined);
  });
...
describe("pet-window-runtime setPetHidden contract (#416)", () => {
  it("reports actual visibility changes for the Settings switch", () => {
    const changes = [];
