...
describe("state-visual-resolver SVG overrides", () => {
  ...
  it("resolves idle, working, juggling, thinking, and null overrides", () => {
    const sessions = new Map([
    ...
    assert.strictEqual(getSvgOverride("thinking", options), "thinking.svg");
  ...
  it("idleDefaultVisual wins over idleFollowSvg", () => {
    const options = {
      idleFollowSvg: "idle-follow.svg",
      idleDefaultVisual: "idle-reading.svg",
    };
    assert.strictEqual(getSvgOverride("idle", options), "idle-reading.svg");
  });
});
