...
describe("i18n locales", () => {
  ...
  it("does not retain copy for the retired primary-display action", () => {
    for (const locale of Object.values(i18n)) {
      assert.strictEqual(Object.hasOwn(locale, "bringPetToPrimaryDisplay"), false);
    }
  });

  it("does not retain copy for the redundant Agent settings shortcut", () => {
    for (const locale of Object.values(i18n)) {
      assert.strictEqual(Object.hasOwn(locale, "openAgentIntegrations"), false);
    }
  });

  it("uses the two short pet menu labels in Chinese", () => {
    assert.strictEqual(i18n.zh.openDashboard, "宠物状态");
    assert.strictEqual(i18n.zh.openAgentLog, "应用界面");
    assert.strictEqual(i18n.en.openDashboard, "Pet Status");
    assert.strictEqual(i18n.en.openAgentLog, "App Window");
  });
  ...
  it("provides the requested Simplified Chinese mode confirmation copy", () => {
    assert.deepStrictEqual(
      ...
      },
      {
        ...
        appModeAutomaticConfirmDetail: "莫比 Pet 将在本次运行期间自动批准新的 Agent 权限请求。已有请求不会被处理，退出应用后授权自动失效。",
