...
describe("visible margin envelopes", () => {
  ...
  it("accepts an explicit box for the theme anchor", () => {
    const clawd = themeLoader.loadTheme("clawd");
    const box = { x: -2, y: -1, width: 12, height: 11 };
    assert.deepStrictEqual(computeThemeAnchorRect(clawd, bounds, { box }),
      hitGeometry.getContentRectScreen(clawd, bounds, "idle", clawd.states.idle[0], { box }));
  });
  ...
  it("still returns an anchor without layout when an explicit box is provided", () => {
    const theme = structuredClone(themeLoader.loadTheme("clawd"));
    delete theme.layout;
    const box = { x: 0, y: 0, width: 20, height: 10 };

    assert.deepStrictEqual(
      computeThemeAnchorRect(theme, bounds, { box }),
