...
function createRuntime(options = {}) {
  ...
  const fs = {
    existsSync(filePath) {
      return /(?:assets[\\/]brand[\\/]icon\.png|resources[\\/]icon\.png)$/.test(filePath);
    },
  };
...
test("settings window runtime creates the Settings BrowserWindow with taskbar identity", () => {
  ...
  assert.strictEqual(win.options.title, "Mobi Agent Pet Settings");
  ...
  assert.strictEqual(win.options.backgroundColor, "#f6f8fc");
  ...
  assert.match(win.options.icon, /assets[\\/]brand[\\/]icon\.png$/);
  assert.strictEqual(win.menuBarVisible, false);
  assert.strictEqual(win.loadedFile, "C:\\app\\src\\settings.html");
  assert.match(win.appDetails.appIconPath, /assets[\\/]brand[\\/]icon\.png$/);
