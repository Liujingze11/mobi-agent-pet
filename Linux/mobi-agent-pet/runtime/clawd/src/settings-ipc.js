...
function registerSettingsIpc(options = {}) {
  ...
  const aboutHeroImagePath = options.aboutHeroImagePath
  ...
  handle("settings:get-pet-hidden", () =>
    typeof options.getPetHidden === "function" && options.getPetHidden() === true);
  handle("settings:set-pet-hidden", (_event, hidden) => {
    if (typeof hidden !== "boolean") {
      return { status: "error", message: "pet visibility requires a boolean" };
    }
    if (typeof options.setPetHidden !== "function" || typeof options.getPetHidden !== "function") {
      return { status: "error", message: "pet visibility is unavailable" };
    }
    try {
      const result = options.setPetHidden(hidden);
      if (!result || !result.applied) {
        return { status: "error", message: result && result.deferred
          ? "pet visibility is changing; try again" : "pet window is unavailable" };
      }
      return { status: "ok", hidden: options.getPetHidden() === true };
    } catch (error) {
      return { status: "error", message: error.message || "pet visibility failed" };
    }
  });
  ...
  handle("settings:get-about-info", () => {
    let heroImageSrc = "";
    try {
      heroImageSrc = pathToFileURL(aboutHeroImagePath).href;
    } catch (err) {
      console.warn("Clawd: failed to resolve about hero image:", err && err.message);
    }
