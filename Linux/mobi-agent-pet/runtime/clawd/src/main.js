...
const _appModeController = createAppModeController({
  ...
  hidePet: () => petWindowRuntime.setPetHidden(true),
  showPet: () => petWindowRuntime.setPetHidden(false),
...
// New installations start in Simplified Chinese. Existing installations keep
// the language saved in their preferences, including any later user changes.
function hydrateFreshInstallLanguage() {
  if (!_initialPrefsLoad || !_initialPrefsLoad.fresh) return;
  if (_settingsController.get("lang") !== "zh") {
    _settingsController.applyUpdate("lang", "zh");
  }
}
...
const petWindowRuntime = createPetWindowRuntime({
  ...
  onPetHiddenChanged: (hidden) => broadcastSettingsWindow("settings:pet-hidden-changed", hidden),
...
function getEffectiveCurrentPixelSize(overrideWa) {
  if (keepSizeAcrossDisplaysCached && isProportionalMode()) {
    ...
    // overrideWa — callers like sendToDisplay pass a *target* display's work
    // area, and seeding from that would freeze the
...
// Pet-anchored floating windows (permission bubbles and session HUD) read the
// scale of whichever display the pet is on right now.
...
function stopTrayFlash() {
  ...
  if (trayStatusController) {
    trayStatusController.acknowledge();
    return;
  }
...
function hasPendingPermissionForTray() {
  return hasPendingTrayQuestion(pendingPermissions);
}

function syncTrayStatus(state) {
  if (!trayStatusController) return;
  let nextState = state;
  if (!nextState && _state && typeof _state.getCurrentState === "function") {
    nextState = _state.getCurrentState();
  }
  trayStatusController.setStatus(resolveTrayStatus({
    state: nextState,
    hasPendingPermission: hasPendingPermissionForTray(),
    enabled: !doNotDisturb && getEffectiveAppModePolicy().suppressTrayFlash !== true,
  }));
}
...
const _permCtx = {
  ...
  syncImeEditingPetDodge: () => topmostRuntime.syncImeEditingPetDodge(),
  onPermissionsChanged: () => syncTrayStatus(),
...
const _stateCtx = {
  ...
  playSound,
  flashTaskbar,
  onStateChanged: (state) => syncTrayStatus(state),
  onTrayAgentEvent: (input) => {
    if (!trayStatusController || !shouldPulseTrayForAgentEvent(input)) return;
    if (doNotDisturb || getEffectiveAppModePolicy().suppressTrayFlash === true) return;
    if (hasPendingPermissionForTray()) return;
    trayStatusController.activity("working");
  },
  t: (key) => t(key),
...
trayStatusController = createTrayStatusLightController({
  setIcon: (icon) => reportTrayOperationFailure(
    "Clawd: tray status update failed:",
    () => _menu.setTrayIcon(icon),
  ),
  completionIntervalMs: _settingsController.get("flashIntervalMs") || 500,
  completionDurationMs: Number(_settingsController.get("flashDurationMs")) > 0
    ? Number(_settingsController.get("flashDurationMs"))
    : 3000,
});
syncTrayStatus();
...
registerSettingsIpc({
  ...
  setPetHidden: (hidden) => {
    if (!hidden) prepManualPetVisibility();
    return petWindowRuntime.setPetHidden(hidden);
  },
  getAllAgents,
  getHookServerPort: () => getHookServerPort(),
  ...
  aboutHeroImagePath: path.join(__dirname, "..", "..", "..", "assets", "brand", "icon.png"),
...
if (!gotTheLock) {
  ...
  app.on("second-instance", (_event, commandLine) => {
    if (getEffectiveAppModePolicy().hidePet) {
      // Background mode owns pet visibility; a second launch must not pull
      // the pet back onto the screen.
    } else if (petWindowRuntime.isPetHidden()) {
      prepManualPetVisibility();
  ...
  const trayShutdownGate = createTrayShutdownGate({
    stop: () => {
      if (trayStatusController) trayStatusController.stop();
      return trayRuntime.stop();
    },
