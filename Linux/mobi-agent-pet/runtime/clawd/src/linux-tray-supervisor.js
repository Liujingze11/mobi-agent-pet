...
function createLinuxTraySupervisor(deps) {
  ...
  async function startFallback(code, { keepNative = false } = {}) {
    if (!keepNative) await stopNative();
    if (!active) return false;
    try {
      await fallback.start(menuSnapshot);
      if (typeof fallback.setIcon === "function") await fallback.setIcon(icon);
      else if (typeof fallback.setAttention === "function") {
        await fallback.setAttention(icon === "agentlog-pet-attention");
      }
  ...
  function scheduleRestart(code) {
    ...
    restartAttempts = Math.min(restartAttempts + 1, RESTART_DELAYS_MS.length - 1);
  ...
  async function handleNoHost(expectedGeneration, expectedChild) {
    ...
    // The helper watches D-Bus for the host to return, even while X11 uses fallback.
    if (String(sessionType).toLowerCase() === "x11") {
      const fallbackStarted = await startFallback(code, { keepNative: true });
      if (!fallbackStarted) transition(STATES.FAILED, "tray-backends-unavailable");
      return;
    }
  ...
  async function activateNative(expectedGeneration, expectedChild) {
    ...
    const wasNative = health.status === STATES.NATIVE;
    clearTimer("startup");
    clearTimer("host-loss");
    await stopFallback();
    ...
    if (!wasNative) {
      stableTimer = timers.setTimeout(() => {
        stableTimer = null;
        if (isCurrent(expectedGeneration, expectedChild) && health.status === STATES.NATIVE) {
          restartAttempts = 0;
        }
      }, STABLE_NATIVE_MS);
    }
  ...
  return {
    ...
    setIcon(nextIcon) {
      return enqueue(async () => {
        if (!protocol.ICON_NAMES || !protocol.ICON_NAMES.has(nextIcon)) {
          throw new TypeError("unsupported tray icon");
        }
        icon = nextIcon;
        iconRevision += 1;
        if (child) {
        ...
        if (fallback.isActive()) {
          if (typeof fallback.setIcon === "function") await fallback.setIcon(icon);
          else await fallback.setAttention(icon === "agentlog-pet-attention");
        }
      });
    },

    setAttention(nextAttention) {
      return this.setIcon(nextAttention === true ? "agentlog-pet-attention" : "agentlog-pet");
    },
