...
function migrate(raw) {
  ...
  // Historical schema revision retained so existing preference files continue
  // through later migrations after the inherited updater fields were removed.
  if (out.version < 6) {
    out.version = 6;
  }
...
//            Callers use this to seed first-run-only state (e.g. UI language)
//            without ever overriding an existing user's choices.
...
function load(prefsPath) {
  ...
  try {
    ...
    // Missing file is normal on first run; return defaults and flag it as fresh.
...
// Map an OS locale string onto one of the supported UI languages. Kept as a pure
// utility for integrations that need locale mapping; unknown locales use English.
