...
let petHiddenChangedDuringBootstrap = false;
if (window.settingsAPI && typeof window.settingsAPI.onPetHiddenChanged === "function") {
  window.settingsAPI.onPetHiddenChanged((hidden) => {
    petHiddenChangedDuringBootstrap = true;
    core.runtime.petHidden = hidden === true;
    if (core.state.activeTab === "general") {
      core.tabs.general.patchInPlace({ petHidden: hidden });
    }
  });
}
...
if (window.settingsAPI && typeof window.settingsAPI.getSnapshot === "function") {
  ...
  const accessoryOptionsPromise =
    typeof window.settingsAPI.getPetAccessoryOptions === "function"
      ...
      : Promise.resolve([]);
  const petHiddenPromise =
    typeof window.settingsAPI.getPetHidden === "function"
      ? window.settingsAPI.getPetHidden().catch((err) => {
        console.warn("settings: getPetHidden failed", err);
        return false;
      })
  ...
  Promise.all([
    ...
    petHiddenPromise,
  ]).then(([snapshot, petTintOptions, petAccessoryOptions, petHidden]) => {
    if (!petHiddenChangedDuringBootstrap) core.runtime.petHidden = petHidden === true;
