...
//   getPetHidden()                      Promise<boolean> (live window state)
//   setPetHidden(hidden)                Promise<{ status, hidden?, message? }>
//   onPetHiddenChanged(cb)             cb(hidden) when visibility changes elsewhere
...
//
// Persisted preference writes go through the main-process "settings:update"
// handler and the controller. Pet visibility is live window state; its dedicated
// IPC calls use the same setter as tray/shortcut actions and broadcast changes.
...
const petHiddenListeners = new Set();
...
ipcRenderer.on("settings:pet-hidden-changed", (_event, hidden) => {
  for (const cb of petHiddenListeners) {
    try { cb(hidden); } catch (err) { console.warn("pet visibility listener threw:", err); }
  }
});
...
contextBridge.exposeInMainWorld("settingsAPI", {
  ...
  getPetHidden: () => ipcRenderer.invoke("settings:get-pet-hidden"),
  setPetHidden: (hidden) => ipcRenderer.invoke("settings:set-pet-hidden", hidden),
  ...
  onPetHiddenChanged: (cb) => {
    if (typeof cb !== "function") return () => {};
    petHiddenListeners.add(cb);
    return () => petHiddenListeners.delete(cb);
  },
