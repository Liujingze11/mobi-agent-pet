...
| **Linux 托盘宿主不可用** | AgentLog 使用桌面提供的 StatusNotifier/AppIndicator 托盘宿主；在没有兼容宿主的桌面上不会要求修改 GNOME 设置，也不会写入 `custom-icons`。X11 可回退到 Electron 托盘；Wayland 会在诊断中显示无托盘宿主。无论是否显示托盘，都可以从应用启动器重新打开 Manager。 |
