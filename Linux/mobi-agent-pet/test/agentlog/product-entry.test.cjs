...
test("root package exposes one Mobi Agent Pet Electron application", () => {
  assert.equal(pkg.name, "mobi-agent-pet");
  assert.equal(pkg.productName, "Mobi Agent Pet");
  assert.equal(pkg.desktopName, "mobi-agent-pet.desktop");
  assert.equal(pkg.main, "runtime/agentlog/main.cjs");
  assert.equal(pkg.engines.node, ">=22.12.0");
  assert.equal(pkg.homepage, "https://github.com/Liujingze11/mobi-agent-pet");
  assert.equal(pkg.build.appId, "com.agentlog.pet");
  assert.equal(pkg.build.productName, "Mobi Agent Pet");
  assert.equal(
    pkg.build.linux.artifactName,
    "Mobi-Agent-Pet-${version}-x64.${ext}"
  );
...
test("the product does not ship the inherited self-update feature", () => {
  assert.equal(Object.hasOwn(pkg.dependencies, "electron-updater"), false);

  for (const relativePath of [
    "runtime/clawd/src/updater.js",
    "runtime/clawd/src/update-bubble.js",
    "runtime/clawd/src/update-bubble.html",
    "runtime/clawd/src/preload-update-bubble.js",
  ]) {
    assert.equal(fs.existsSync(path.join(root, relativePath)), false, `obsolete updater asset remains: ${relativePath}`);
  }

  const activeSources = [
    "runtime/clawd/src/main.js",
    "runtime/clawd/src/menu.js",
    "runtime/clawd/src/tray-menu-model.js",
    "runtime/clawd/src/preload-settings.js",
    "runtime/clawd/src/settings-ipc.js",
    "runtime/clawd/src/settings-tab-about.js",
  ].map((relativePath) => fs.readFileSync(path.join(root, relativePath), "utf8")).join("\n");

  assert.doesNotMatch(activeSources, /checkForUpdates|setupAutoUpdater|getUpdateMenuItem|settings:check-for-updates|updates\.action/);
});
...
test("the selected brand is the only application logo source", () => {
  const legacyLogoPaths = [
    ...
    "runtime/clawd/assets/tray-icon-flash.png",
...
test("every active application logo matches the approved current brand", () => {
  const approvedRoot = path.join(root, "assets", "source", "brand-logo-current");
  const activePairs = [
    ["brand/icon.png", "assets/brand/icon.png"],
    ...[16, 32, 48, 64, 128, 256, 512].map((size) => [
      `brand/icons/${size}x${size}.png`,
      `assets/brand/icons/${size}x${size}.png`,
    ]),
    ["pwa/icon-256.png", "runtime/clawd/pwa/icons/icon-256.png"],
    ["pwa/icon-512.png", "runtime/clawd/pwa/icons/icon-512.png"],
    ...[16, 22, 32, 48].flatMap((size) => [
      [`tray/${size}x${size}/agentlog-pet.png`, `assets/tray-icons/hicolor/${size}x${size}/status/agentlog-pet.png`],
      [`tray/${size}x${size}/agentlog-pet-panel.png`, `assets/tray-icons/hicolor/${size}x${size}/status/agentlog-pet-panel.png`],
    ]),
  ];

  for (const [approvedRelative, activeRelative] of activePairs) {
    const approved = path.join(approvedRoot, approvedRelative);
    const active = path.join(root, activeRelative);
    assert.equal(fs.existsSync(approved), true, `missing approved logo: ${approvedRelative}`);
    assert.equal(fs.existsSync(active), true, `missing active logo: ${activeRelative}`);
    assert.equal(digest(active), digest(approved), `stale active logo: ${activeRelative}`);
  }

  assert.equal(
    pkg.build.files.some((entry) => entry.startsWith("assets/source/")),
    false,
    "brand source and history must not be packaged",
  );
  assert.deepEqual(
    fs.readdirSync(path.join(root, "assets", "source", "brand-logo-history")).sort(),
    ["README.md", "legacy-status-assets", "v1-devpulse-purple", "v2-blue-ring-white", "v3-blue-ring-transparent"],
    "the history folder should contain only retired artwork",
  );
});
...
test("smoke mode reports only ready Mobi Agent Pet and tray diagnostics", async () => {
  const writes = [];
  let readyCallback;
  let timerUnrefCalled = false;
  let agentLogInstallCalls = 0;
  const fakeApp = {
    getPath(name) { assert.equal(name, "appData"); return "/private/user"; },
    setPath(name, value) { assert.equal(name, "userData"); assert.equal(value, "/private/user/AgentLog Pet"); },
    setName(name) {
      this.name = name;
    },
    getName() {
  ...
  const context = vm.createContext({
    ...
    require(request) {
      if (request === "node:fs") return { mkdirSync() {} };
      if (request === "electron") {
        return {
          app: fakeApp,
          BrowserWindow: {
      ...
      if (request === "./brand.cjs") {
        return { BRAND: { productName: "Mobi Agent Pet" }, legacyUserDataPath: (dir) => path.join(dir, "AgentLog Pet") };
      }
  ...
  assert.deepEqual(writes, [
    'AGENTLOG_SMOKE_READY {"productName":"Mobi Agent Pet","windowCount":1,"pid":4242,"tray":{"status":"native","code":null}}\n',
  ]);
...
test("manager smoke mode opens the real manager and reports only safe rendered metadata", async () => {
  ...
  const context = vm.createContext({
    ...
    require(request) {
      ...
      if (request === "./brand.cjs") return { BRAND: { productName: "Mobi Agent Pet" }, legacyUserDataPath: (dir) => path.join(dir, "AgentLog Pet") };
  ...
  assert.deepEqual(writes, [
    'AGENTLOG_MANAGER_SMOKE_READY {"status":"ok","pid":4242,"productName":"Mobi Agent Pet","managerTitle":"Mobi Agent Pet","databaseName":"agentlog.db","managerShell":true}\n',
  ]);
...
if (args.includes("--ozone-platform=x11")) {
  ...
  server.listen(socketPath, () => {
    process.stdout.write("AGENTLOG_SMOKE_READY " + JSON.stringify({ productName: "Mobi Agent Pet", windowCount: 1, pid: process.pid, tray: JSON.parse(process.env.AGENTLOG_SMOKE_FIXTURE_TRAY) }) + "\\n");
  });
}
`);
