...
const AGENTLOG_REMOVED_UPSTREAM_FILES = [
  "src/preload-update-bubble.js",
  "src/update-bubble.html",
  "src/update-bubble.js",
  "src/updater.js",
  "test/permission-update-bubble-ipc.test.js",
  "test/update-bubble-autoclose.test.js",
...
test("the complete Clawd runtime is pinned to the approved commit", () => {
  ...
  assert.deepEqual(manifest.agentLogRemovedFiles, AGENTLOG_REMOVED_UPSTREAM_FILES);
  ...
  for (const relativePath of manifest.agentLogRemovedFiles) {
    assert.equal(
      fs.existsSync(path.join(runtimeRoot, relativePath)),
      false,
      `removed upstream file was restored: ${relativePath}`
    );
  }
});
