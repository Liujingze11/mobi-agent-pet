...
test("untagged RangeErrors are redacted through the envelope and preload", async (t) => {
  ...
  assert.deepEqual(await harness.invokeEnvelope("agentlog:projects:archive", { id: "project-1" }), {
    ok: false,
    error: { code: "INTERNAL_ERROR", message: "Mobi Agent Pet operation failed" },
  });
  await assert.rejects(
    () => renderer.projects.archive("project-1"),
    (error) => error.code === "INTERNAL_ERROR"
      && error.message === "Mobi Agent Pet operation failed"
...
test("unexpected service errors retain a structured code without leaking their message", async (t) => {
  ...
  await assert.rejects(
    ...
    (error) => error.code === "INTERNAL_ERROR" && error.message === "Mobi Agent Pet operation failed"
  );
});
...
test("diagnostics emits only allowlisted tray health", async (t) => {
  let runtimeHealth = {
    ...
    errorMessage: "Unable to open Mobi Agent Pet storage at /home/user/.config",
