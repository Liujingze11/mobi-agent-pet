...
test("the repository contains forty structurally valid stable tray icons in a GTK theme", () => {
  const theme = fs.readFileSync(
    path.join(ROOT, "assets", "tray-icons", "hicolor", "index.theme"),
    "utf8",
  );
  assert.match(theme, /^\[Icon Theme\]$/m);
  for (const size of ICON_SIZES) {
    assert.match(theme, new RegExp(`^\\[${size}x${size}\\/status\\]$`, "m"));
  }
  for (const size of ICON_SIZES) {
...
test("the question light preserves the approved solid yellow artwork exactly", () => {
  for (const size of ICON_SIZES) {
    ...
    assert.deepEqual(
      fs.readFileSync(path.join(directory, "agentlog-pet-question.png")),
      approvedYellow,
      `${size}px question light must reuse the approved yellow icon bytes`,
    );
    assert.deepEqual(
      fs.readFileSync(path.join(directory, "agentlog-pet-question-panel.png")),
      approvedYellow,
      `${size}px panel alias must reuse the approved yellow icon bytes`,
    );
  }
});
    ...
    try {
      ...
      assert.deepEqual(
        ...
        [
          ...
          "the repository contains forty structurally valid stable tray icons in a GTK theme",
          "the question light preserves the approved solid yellow artwork exactly",
        ],
      );
      assert.equal(outcomes.length, 17, "the child must report every package test");
      assert.equal(
