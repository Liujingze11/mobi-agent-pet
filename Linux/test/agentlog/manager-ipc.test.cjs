...
test("unexpected service errors retain a structured code without leaking their message", async (t) => {
  ...
  await assert.rejects(
    ...
    (error) => error.code === "SQLITE_BUSY" && error.message === "AgentLog operation failed"
  );
});
...
test("preload exposes only frozen named APIs and unsubscribes its own change listener", () => {
  const { installPreload } = require("../../runtime/agentlog/manager/preload.cjs");
  const invocations = [];
  const listeners = [];
  let exposed;
  installPreload({
    ...
    ipcRenderer: {
      invoke(...input) { invocations.push(input); },
