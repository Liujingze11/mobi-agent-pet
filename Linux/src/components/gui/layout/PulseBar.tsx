...
export default function PulseBar() {
  ...
  const statusLabel: Record<string, string> = {
    idle: '空闲', working: '工作中', learning: '学习中', deep_focus: '深度专注', paused: '已暂停'
  }
  const statusColor: Record<string, string> = {
    idle: 'bg-slate-500', working: 'bg-blue-500', learning: 'bg-green-500', deep_focus: 'bg-amber-500', paused: 'bg-yellow-500'
  }
  ...
  return (
    <div className="bg-slate-800/80 border-b border-slate-700 px-6 py-2.5 flex items-center gap-4">
      ...
      {/* 计时器 */}
      {!isIdle && (
        <div className="font-mono text-lg font-bold text-white min-w-[80px]">
          {fmt(tick.effectiveMs)}
        </div>
      )}

      {/* 今日统计 */}
      <div className="text-xs text-slate-400 flex gap-3 min-w-[160px]">
        <span>研发 {fmtMin(tick.todayWorkMinutes)}</span>
        <span>学习 {fmtMin(tick.todayLearningMinutes)}</span>
      </div>
      ...
      {showPicker === 'work' && (
        <div className="absolute top-full right-6 mt-2 bg-slate-800 border border-slate-700 rounded-xl p-3 shadow-2xl z-50 w-56 space-y-2">
          ...
          <button onClick={handleStartWork} disabled={!selProject}
            className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 text-white rounded-lg py-2 text-xs font-medium">开始</button>
        </div>
      )}
      ...
      {showPicker === 'learning' && (
        <div className="absolute top-full right-6 mt-2 bg-slate-800 border border-slate-700 rounded-xl p-3 shadow-2xl z-50 w-56 space-y-2">
          ...
          <button onClick={handleStartLearning} disabled={!selTopic}
            className="w-full bg-green-600 hover:bg-green-500 disabled:bg-slate-700 text-white rounded-lg py-2 text-xs font-medium">开始</button>
        </div>
      )}
    </div>
  )
